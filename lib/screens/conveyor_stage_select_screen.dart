...
import 'conveyor_gameplay_screen.dart';
...
const conveyorLevelSelectRouteName = 'conveyor-level-select';

/// Seleção de Fases do Mundo 2 (Esteira de Bugs) — mesmo papel de
/// `LevelSelectScreen`, adaptado para `ConveyorLevel`/`world2Levels`. O
/// progresso (`Progress.instance`) só dura a sessão atual do app —
/// persistência entre sessões ainda não foi decidida (ver
/// `.claude/memory/decisions.md`).
class ConveyorStageSelectScreen extends StatefulWidget {
  final GameWorld world;

  const ConveyorStageSelectScreen({super.key, required this.world});

  @override
  State<ConveyorStageSelectScreen> createState() => _ConveyorStageSelectScreenState();
}

class _ConveyorStageSelectScreenState extends State<ConveyorStageSelectScreen> {
  ...
  // esteira (Mundo 2), então converte de volta para `List<ConveyorLevel>`.
  List<ConveyorLevel> get _levels => widget.world.levels.cast<ConveyorLevel>();
  ...
  void _openLevelAt(int index) async {
    await Navigator.of(context).push(MaterialPageRoute(builder: (_) => ConveyorGameplayScreen(level: _levels[index])));
