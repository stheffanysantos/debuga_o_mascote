...
import 'code_puzzle_gameplay_screen.dart';
...
/// Seleção de Fases do Mundo 3 (Modo Debug) — mesmo papel de
/// `LevelSelectScreen`/`ConveyorStageSelectScreen`, adaptado para
/// `CodePuzzleLevel`/`world3Levels`. O progresso (`Progress.instance`) só
/// dura a sessão atual do app — persistência entre sessões ainda não foi
/// decidida (ver `.claude/memory/decisions.md`).
class CodePuzzleStageSelectScreen extends StatefulWidget {
  final GameWorld world;

  const CodePuzzleStageSelectScreen({super.key, required this.world});

  @override
  State<CodePuzzleStageSelectScreen> createState() => _CodePuzzleStageSelectScreenState();
}

class _CodePuzzleStageSelectScreenState extends State<CodePuzzleStageSelectScreen> {
  ...
  // puzzles de código (Mundo 3), então converte de volta para
  // `List<CodePuzzleLevel>`.
  List<CodePuzzleLevel> get _levels => widget.world.levels.cast<CodePuzzleLevel>();
  ...
  void _openLevelAt(int index) async {
    await Navigator.of(context).push(MaterialPageRoute(builder: (_) => CodePuzzleGameplayScreen(level: _levels[index])));
