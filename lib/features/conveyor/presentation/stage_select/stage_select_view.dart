...
import '../gameplay/conveyor_gameplay_view.dart';
...
/// (Mundo 1, `lib/features/maze/presentation/stage_select/stage_select_view.dart`).
const conveyorLevelSelectRouteName = 'conveyor-level-select';

/// Seleção de Fases do Mundo 2 (Esteira de Bugs) — mesmo papel de
/// `StageSelectView` (Mundo 1), adaptado para `ConveyorLevel`/
/// `world2Levels`. Sem lógica de orquestração própria — `ConsumerWidget`,
/// sem ViewModel/State dedicados.
class ConveyorStageSelectView extends ConsumerWidget {
  final GameWorld world;

  const ConveyorStageSelectView({super.key, required this.world});
  ...
  List<ConveyorLevel> get _levels => world.levels.cast<ConveyorLevel>();
  ...
  void _openLevelAt(BuildContext context, int index) {
    Navigator.of(context).push(MaterialPageRoute(builder: (_) => ConveyorGameplayView(levelId: _levels[index].id)));
  }
