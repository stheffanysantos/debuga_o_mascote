...
import '../gameplay/block_program_gameplay_view.dart';
...
/// Mundo 1). Só o Mundo 4 ("Decisões em Bloco") usa este motor hoje — o
/// mundo irmão original ("Oficina de Blocos") saiu do jogo, ver
/// `.claude/memory/decisions.md`, entrada de 2026-09-18.
const blockProgramLevelSelectRouteName = 'block-program-level-select';

/// Seleção de Fases do Mundo 4 ("Decisões em Bloco") — mesmo papel de
/// `StageSelectView` (Mundo 1), adaptado para `BlockProgramLevel`. Sem
/// lógica de orquestração própria — `ConsumerWidget`, sem ViewModel/State
/// dedicados.
class BlockProgramStageSelectView extends ConsumerWidget {
  final GameWorld world;

  const BlockProgramStageSelectView({super.key, required this.world});
  ...
  // "Programação em Blocos" (Mundo 4), então converte de volta para
  // `List<BlockProgramLevel>`.
  List<BlockProgramLevel> get _levels => world.levels.cast<BlockProgramLevel>();
  ...
  void _openLevelAt(BuildContext context, int index) {
    Navigator.of(context).push(MaterialPageRoute(builder: (_) => BlockProgramGameplayView(levelId: _levels[index].id)));
  }

  /// Reabre a `TutorialView` deste mundo — botão "?" do cabeçalho.
